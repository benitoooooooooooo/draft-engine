...
from optimize_lineup import BUN, EVAL, JS, proj_from_cell, pos_from_cell, season_weekly, load_draft_positions
...
def main():
    ...
    for p in wire:
        ...
        val = weekly.get(p['name'], 0.0)
        if pos and val > 0:
            rows.append((p['name'], pos, val))
    rows.sort(key=lambda x: -x[2])
    seen, top = set(), []
    for name, pos, val in rows:
        if name not in seen:
            seen.add(name)
            top.append((name, pos, val))

    L = ['🌊 WEEKLY WAIVER SWEEP — Straight to Jail', '─' * 44]
    if injured:
        L.append('⚠ Our injuries: ' + ', '.join(f'{n} [{i}]' for n, i, _ in injured))
    L.append('Best on the wire (season-avg/week):')
    for name, pos, val in top[:12]:
        L.append(f'  {pos:<4} {name:<26} {val:>5.1f}')
    L.append('─' * 44)
